    ...
    public class ManageController : Controller
    {
        ...
        public async Task<ActionResult> Index(ManageMessageId? message)
        {
            ViewBag.StatusMessage =
                ...
                : message == ManageMessageId.SetTwoFactorSuccess ? "Налаштований постачальник двофакторної перевірки автентичності."
                : message == ManageMessageId.Error ? "Сталася помилка."
                : message == ManageMessageId.AddPhoneSuccess ? "Ваш номер телефону додано."
                : message == ManageMessageId.RemovePhoneSuccess ? "Ваш номер телефону видалений."
                : message == ManageMessageId.AddShopSuccess ? "Адресу збережено."
                : "";
        ...
        public async Task<ActionResult> AddPhoneNumber(AddPhoneNumberViewModel model)
        {
            ...
            ModelState.AddModelError("", "Не вдалося додати телефон");
            return View(model);
        ...
        // POST: /Manage/UserShopAdress
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> UserShopAdress(IndexViewModel model)
        {
            await SaveShop(model.Shop, model.User.Id);

            return RedirectToAction("Index", new { Message = ManageMessageId.AddShopSuccess });
        }

        // POST: /Manage/UserAdress
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> UserAdress(IndexViewModel model)
        {
            await SaveAdress(model.Adress, model.User.Id);
        ...
        private async Task<Shop> GetShop(string userId)
        {
            var shop = await AdressManager.GetShopByUserId(userId) ?? AdressManager.CreateShop(null);
            if (!string.IsNullOrEmpty(shop.AdressId)) shop.Adress = await AdressManager.GetAdress(shop.AdressId);
            return shop;
        }
